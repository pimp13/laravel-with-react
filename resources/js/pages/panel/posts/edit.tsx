...
export default function CreatePostPage({ postDetails }: { postDetails: any }) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-slate max-w-none min-h-[440px] p-5 focus:outline-none text-right",
        dir: "rtl",
      },
    },
  });
  ...
  const seoChecks = useMemo<SeoCheck[]>(() => {
    ...
    const firstParagraphHasKeyword =
      !!focusKeyword && normalizeText(firstParagraph).includes(focusKeyword);
    ...
    return [
      ...
      },
      {
        ...
        description: "کلمه کلیدی باید موضوع اصلی مقاله را واقعاً نمایندگی کند.",
        passed: !!focusKeyword,
        important: true,
        points: 7,
      },
      ...
      },
      {
        ...
        description: "متن جایگزین باید توصیفی و مرتبط با تصویر واقعی باشد.",
      ...
      },
      {
        ...
        description: "در صورت وجود صفحات مرتبط، به آن‌ها لینک داخلی بدهید.",
      ...
      },
      {
        ...
        description: "در صورت نیاز به منبع، به منابع معتبر و مرتبط ارجاع دهید.",
  ...
  */
  ...
  //   }
  // };
  const createNewPostLogic = async () => {
    if (!form.title.trim()) {
      window.alert("عنوان مقاله الزامی است.");
      return;
    }
  ...
  const handleSubmit = async (event: React.SubmitEvent<HTMLFormElement>) => {
    ...
    await createNewPostLogic();
  };

  const handleSaveToDraft = async (e: React.MouseEvent<HTMLButtonElement>) => {
    setField("visibility", "draft");
    await createNewPostLogic();
  };
  ...
  return (
    <PanelLayout title="مشاهده پست">
      <div dir="rtl" className="min-h-screen ">
        <header className="sticky top-7 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
